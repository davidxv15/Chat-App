...
const ActiveUsers: React.FC<{ room: string }> = ({ room }) => {
  ...
  return (
    <div className={`active-users-list hover:scale-125 bg-gray-800 bg-opacity-80 text-white px-2 pl-4 py-0 sticky  dark:bg-opacity-5 backdrop-blur-sm dark:text-yellow-100 transition-all duration-600 ease-in-out ${isScrolled ? 'transform translate-y-[4.5rem]' : ''}`}>
