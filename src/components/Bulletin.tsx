import React, { useState, useEffect } from "react";

interface Quote {
  author: string;
  text: string;
}

const Bulletin: React.FC = () => {
  const [quote, setQuote] = useState<Quote | null>(null);

  // Fetch the JSON file on component mount
  useEffect(() => {
    fetch("/Quotes.json")
      .then((response) => response.json())
      .then((data) => {
        const quotes = data.quotes;
        const randomIndex = Math.floor(Math.random() * quotes.length);
        setQuote(quotes[randomIndex]);
      })
      .catch((error) => console.error("Error loading quotes:", error));
  }, []);

  return (
    <div
      className="bulletin rounded bg-gray-800 text-center px-2 w-auto py-4 mb-2 mt-4 border border-gray-600"
      role="complementary"
      aria-labelledby="bulletin-title"
      aria-describedby="bulletin-description"
    >
      <h2
        className="text-2xl font-bold text-blue-500 pt-0 pb-2"
        id="bulletin-title"
      >
        {quote ? "Quote of the Day" : "Your Ad Here!"}
      </h2>
      <p id="bulletin-description" className="text-gray-300 p-2">
        {quote
          ? `"${quote.text}" - ${quote.author}`
          : "Visit our sponsors! Advertise here!"}
      </p>
      {/* <p
        className="contact text-white"
        aria-label="Contact for advertisement inquiries"
      >
        Contact <a href="https://www.linkedin.com/in/david-velasquez-az/">David Velasquez</a>
        {" "}
      </p> */}
    </div>
  );
};

export default Bulletin;
